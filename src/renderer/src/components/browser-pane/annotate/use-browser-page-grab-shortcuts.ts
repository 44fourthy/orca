...
/**
 * The element picker's keyboard door: the grab keybinding, the guest-forwarded toggle, and the
 * C/S actions while a pick is live. Split from the browsing pane's shortcut cluster so a surface
 * that renders its own chrome — the client-hosted remote pane — shares one implementation.
 */
export function useBrowserPageGrabShortcuts({
  browserTabId,
  workspaceId,
  chromeShortcutScope,
  markupIsActive,
